...
this.router.get('/employee/:id', this.GpGetEntityById);
...
public GpGetEntityById(req: Request, res: Response) {
            new CustomLogger().showLogger('info', 'Enter into employeeController.ts: GpGetEntityById');
        new ApiAdapter().get(Constant.TESTFEATUREURL + `${req.url}` )
        .then((res: any) => res.response.json()).then(result => {
            switch(req.baseUrl) { case '/mobile': res.send(result); break; case '/web': res.send(result); break; default: res.send(null); }
            new CustomLogger().showLogger('info', 'Exit from employeeController.ts: GpGetEntityById');
        }).catch(err => {
            res.send(err);
        });
    }
